...
LoginApp.controller("CustomerOrdersController", function ($scope, $http, userApi, customerPersist) {
    var customer = customerPersist.getCookieData();
    $scope.customername = customer.customername;
    var customerData = [];
    $scope.consumer = customer.customerid;
    $http({
        method: 'Get',
...
LoginApp.controller("DeliveriesController", function ($scope, $http, userApi, driverPersist) {
    var driver = driverPersist.getCookieData();
    $scope.drivername = driver.drivername;
     var driverData = [];
    $scope.driver = $scope.driverid;
...
LoginApp.controller("SupplierInventoryController", function ($scope, $http, userApi, supplierPersist) {
    var supplier = supplierPersist.getCookieData();
    $scope.suppliername = supplier.suppliername;
    var supplierData = [];
    $scope.supplier = supplier.supplierid;
    ...
    $scope.updateInventory = function (a) {
        var product = {
        ...
        userApi.PutProduct(product).then(function (response) {
            ...
            $scope.ThresholdQuantity = $scope.ThresholdQuantity;
            $scope.BatchQuantity = $scope.BatchQuantity;
            location.reload();
        });
    }
});
...
LoginApp.controller("ProductsController", function ($scope, $http, userApi,adminPersist) {
    ...
    //$scope.products;
    //var prod = $scope.proInfo = [];
    //$scope.proInfo.push($scope.product);
    ...
    $scope.EditProductByID = function (prod) {
        var proInfo = {
...
LoginApp.controller("ImageController", function ($location, $scope, $http, userApi, fileUpload) {
    ...
    $scope.uploadFile = function (ProductID) {
       ...
       fileUpload.uploadFileToUrl(file, prodid, uploadUrl);
        location.reload();
    };
    getImage();
    function getImage () {
...
LoginApp.service('fileUpload', ['$http', function ($http, $scope) {
    this.uploadFileToUrl = function (file, prodid, uploadUrl) {
        var pd = prodid;
